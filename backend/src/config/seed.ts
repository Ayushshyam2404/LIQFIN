...
const seedDatabase = async () => {
  ...
  try {
    ...
    if (userCount > 0 && !shouldClean) {
      console.log('=============================================');
      console.log('  Database already contains user accounts.');
      console.log('  Skipping seed to protect your profile.');
      console.log('  To force re-seeding, run: npm run seed -- --clean');
      console.log('=============================================');
      return;
    }

    if (shouldClean) {
      console.log('Cleaning existing data (forced reset)...');
      ...
      console.log('Database is empty. Initializing fresh seed data...');
    }
    ...
    // Create Credit Cards
    const appleCard = await CreditCard.create({
      userId: demoUser._id,
    ...
    const chaseCard = await CreditCard.create({
      userId: demoUser._id,
    ...
    const amexCard = await CreditCard.create({
      userId: demoUser._id,
      cardName: 'Gold Card',
    ...
    for (const card of cardsList) {
      const totalCardSpentAgg = await Expense.aggregate([
        {
          $match: {
            userId: demoUser._id,
    ...
    const activeBudgets = await Budget.find({ userId: demoUser._id, month: currentMonth });
