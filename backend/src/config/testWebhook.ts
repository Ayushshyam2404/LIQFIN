...
import { handleEmailWebhook } from '../controllers/expenseController';
...
const testWebhook = async () => {
  ...
  try {
    ...
    let user = await User.findOne({ email: 'demo@liquid.finance' });
    if (!user) {
    ...
    await CreditCard.deleteMany({ userId: user._id, cardNumberLastFour: '2629' });
    await Expense.deleteMany({ userId: user._id, tags: 'email-webhook' });
    await Notification.deleteMany({ userId: user._id, title: 'Email Alert Parsed' });
...
`;
    ...
    await handleEmailWebhook(req, res, next);
    ...
    // Verify Card
    const dbCard = await CreditCard.findById(card._id);
    if (!dbCard) {
    ...
    // Verify Expense
    const dbExpense = await Expense.findById(expense._id);
    if (!dbExpense) {
    ...
    const refreshedCard = await CreditCard.findById(card._id);
    ...
    // Verify Budget sync
    const dbBudget = await Budget.findOne({ userId: user._id, category: 'Food', month: currentMonth });
    ...
    // Verify Notification
    const dbNotification = await Notification.findOne({ userId: user._id, title: 'Email Alert Parsed' });
    if (!dbNotification) {
